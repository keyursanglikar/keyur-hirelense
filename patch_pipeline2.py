...
new_content = re.sub(
    r'                  <tbody>\s*\{filteredCandidates\.map\(\(c, idx\) => \{\s*const ini = c\.name \? c\.name\.split\(" "\)\.map\(x => x\[0\]\)\.join\(""\) : "\?\?";\s*const scoreClass = c\.score === null \? "" : c\.score >= 71 \? "s-hi" : c\.score >= 51 \? "s-mid" : "s-lo";\s*return \(\s*<tr key=\{idx\}>\s*<td>\s*<span className="tname">\s*<span className="mini-av">\{ini\}</span>\{c\.name\}\s*</span>\s*</td>\s*<td>\s*<span className=\{\adge \$\{c\.status === \'Shortlisted\' \? \'b-ok\' : c\.status === \'Scored\' \? \'b-info\' : c\.status === \'In progress\' \? \'b-amber\' : c\.status === \'Invited\' \? \'b-mute\' : \'b-rec\'\}\\}>\s*\{c\.status\}\s*</span>\s*</td>\s*<td>\s*\{c\.score !== null \? <span className=\{\scorechip \$\{scoreClass\}\\}>\{c\.score\}</span> : <span className="okcell">—</span>\}',
    replacer,
    content,
    flags=re.DOTALL
)
...
with open(file_path, 'w', encoding='utf-8') as f:
    f.write(new_content)
print("Pipeline patched successfully again!")
