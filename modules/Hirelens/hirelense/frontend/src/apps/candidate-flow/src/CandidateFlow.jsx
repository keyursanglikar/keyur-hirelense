  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (SpeechRecognition && speakingState === 'listening') {
        ...
        try {
          setCurrentTranscript(''); // Reset transcript for new question
          recognition.start();
        } catch (e) {
  ...
  const handleSaveAndNextQuestion = () => {
    ...
    setAnswersList({
      ...
      [`q-${question.id}`]: {
        ...
        videoCaptured: true,
        answer: currentTranscript.trim() || `Candidate's verbal explanation for: ${question.text}`
      }
    });
  ...
  const handleFinishCaseAnswer = () => {
    ...
    setAnswersList({
      ...
      [`q-${question.id}`]: {
        ...
        videoCaptured: true,
        answer: currentTranscript.trim() || `Candidate's verbal explanation for case study: ${question.text}`
      }
    });
    setCurrentTranscript('');
                  ...
                  <div style={{ padding: '16px', border: '1.5px solid var(--line)', borderRadius: '12px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <span className="mono" style={{ fontSize: '11px', color: 'var(--muted)' }}>
                        {micLevel > 0 ? "Mic active" : "Checking..."}
                      </span>
                    </div>
                  </div>
                  ...
                  {isSavingQuestion ? (
                    ...
                    <>
                      ...
                      <div style={{ marginTop: '28px' }}>
                        {speakingState === 'asking' ? (
                          <button className="btn primary" disabled style={{ width: '100%', height: '48px', fontSize: '14px', cursor: 'not-allowed' }}>
                            {isAudioPlaying ? 'Listening to Question...' : `Thinking Time (0:${String(questionTimer).padStart(2, '0')})`}
                          </button>
                        ) : (
                          <button 
                            className="btn primary" 
